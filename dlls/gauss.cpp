...
void CGauss::SecondaryAttack()
{
	...
	else
	{
		if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] > 0)
		{
			...
			if (UTIL_WeaponTimeBase() >= m_pPlayer->m_flNextAmmoBurn && m_pPlayer->m_flNextAmmoBurn != 500)
			{
#ifdef CLIENT_DLL
				if (bIsMultiplayer())
#else
		...
		if (UTIL_WeaponTimeBase() >= m_pPlayer->m_flAmmoStartCharge)
		{
			...
			m_pPlayer->m_flNextAmmoBurn = 500;
		}
...
#ifndef CLIENT_DLL
			m_pPlayer->TakeDamage(CWorld::World->pev, CWorld::World->pev, 35, DMG_SHOCK);
...
void CGauss::StartFire()
{
	...
	{
		flDamage = 100 * ((gpGlobals->time - m_pPlayer->m_flStartCharge) / GetFullChargeTime());
	}
